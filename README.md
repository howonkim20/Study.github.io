...
- 비트, 비트열
      ...
      2 . 비트열이란?
      비트 한개로는 많은 양의 데이터르 표기하기에는 턱없이 부족하므로 여러 숫자조합을 컴퓨터에 나타내기 위해 비트을 사용합니다. 바이트는 여덟개의 비트가 모여 만들어 진것입니다. 하나의 바이트에 여덟 개의 비트가 있고, 비트 하나는 0과 1로 표현될 수 있기 때문에 2^8 = 256 개의 서로 다른 바이트가 존재할 수 있습니다.
...
- 가상 현실(VR) / 증강 현실(AR)
      ...
      가상 현실(Virtual Reality)과 증강 현실(Augmented Reality)은 최근 들어 급격히 전세계적으로 유명해졌습니다. 가상현실을 통해 3차원 세계에서 게임이나 비디오 등의 시뮬레이션을 경험할 수 있게 되었고, 증강현실은 가상현실과 기본적으로 비슷한데 사용자에게 기존의 주변환경과 분리된 전혀 다른 환경을 경험하게 하지 않고 현재의 환경 위에 영상, 게임 등의 효과를 입히는 기술입니다.
